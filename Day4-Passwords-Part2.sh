...
#awk command only retain passport with seven fields
awk 'NF>=1 {printf("%s ",$0)} NF==0 {print}' temp  | sed -e 's/cid:[^ ]*[ \t]/ /; s/cid:[^ ]*$//' | awk 'NF==7' > temp2
...
do
	...
	BEGIN{FS=":";bool=0}
	$1~/^byr$/ {if($2~/[0-9]{4}/ && ($2>=1920) && ($2<=2002)) {bool=1}}
	$1~/^iyr$/ {if($2~/[0-9]{4}/ && ($2>=2010) && ($2<=2020)) {bool=1}}
	$1~/^eyr$/ {if($2~/[0-9]{4}/ && ($2>=2020) && ($2<=2030)) {bool=1}}
	$1~/^hgt$/ {
		    if($2~/^[0-9]+cm$/) {H=substr($2,1,length($2)-2); if(H>=150 && H<=193) {bool=1}}
		    if($2~/^[0-9]+in$/) {H=substr($2,1,length($2)-2); if(H>=59 && H<=76) {bool=1}}
		   }
	$1~/^hcl$/ {if($2~/^#[0-9a-f]{6}$/) {bool=1}}
	$1~/^ecl$/ {if ($2~/^amb$/ || $2~/^blu$/ || $2~/^brn$/ || $2~/^gry$/ || $2~/^grn$/ || $2~/^hzl$/ || $2~/^oth$/ ) {bool=1}}
	$1~/^pid$/ {if($2~/^[0-9]{9}$/) {bool=1}}
	END{print bool}
       	')
